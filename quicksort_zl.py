...
import random
test = random.sample(range(0, 15), 20)
...
def partition(list_to_sort, low_index, high_index):
	...
	for i in range(low_index, high_index):
		...
		if (list_to_sort[i] < list_to_sort[pivot]):
			#swap those two
			list_to_sort[i], list_to_sort[devider] = list_to_sort[devider], list_to_sort[i]
			divider += 1
	#once it sorts from low index to high index
	list_to_sort[pivot], list_to_sort[devider] = list_to_sort[devider], list_to_sort[i]
	return divider
